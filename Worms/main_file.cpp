...
void nextFrame(void) {
	...
	for (int i = 0; i < robots.size(); i++)
	{
		float distance = wall.HowFarFromSurface(robots[i].body.M2, vec4((robots[i].body.boundingBox.bottomLeft.x + robots[i].body.boundingBox.bottomRight.x) / 2, robots[i].body.boundingBox.bottomRight.y, 0.0f, 0.0f));
		cout << distance << endl;
		if (distance > 0.0f)
		{
